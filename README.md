...
This repository contains the official implementation and resources for the paper **"Narrative Consolidation: Formulating a New Task for Unifying Multi-Perspective Accounts"**. The project defines and benchmarks the task of consolidating multiple, overlapping narrative documents into a single, chronologically coherent text guided by a canonical timeline.

Publication
- This work was accepted and published in the Journal of the Brazilian Computer Society (JBCS): "Narrative Consolidation: Formulating a New Task for Unifying Multi-Perspective Accounts" (JBCS 2026). Link: https://journals-sol.sbc.org.br/index.php/jbcs/article/view/7717

## Abstract

Processing overlapping narrative documents, such as legal testimonies or historical accounts, often aims not for compression but for a unified, coherent, and chronologically sound text. Standard multi-document summarization (MDS) methods prioritize conciseness and semantic centrality, which can harm chronological integrity and completeness when fusing multiple perspectives. Narrative Consolidation reframes the problem: the objective is to produce a single narrative that is complete, coherent, and temporally consistent with a canonical timeline.

Benchmarking systems on the Gospel Consolidation dataset yields three key findings: first, the explicit temporal backbone (a canonical timeline) is the dominant factor — granting any system the canonical timeline greatly improves ROUGE-L and temporal coherence; second, a simple length heuristic is a surprisingly strong reference for fusion-style references; third, an explicit relational prior (the TAEG) yields signal but does not by itself beat simple heuristics on the chosen reference.
...
Traditional MDS focuses on conciseness. In contexts like criminal investigations with multiple witness testimonies or historical analysis with multiple sources, the priority is often chronological completeness and coherence rather than compression. Classic graph-based algorithms like LexRank optimize for semantic centrality and therefore can produce outputs that disrupt the chronological flow of events.
...
A standard graph-based extractive summarization approach that treats the four Gospels as a single multi-document collection: sentences are nodes and edges are weighted by sentence similarity. This baseline is timeline-agnostic and optimizes semantic centrality.
...
The TAEG is a structured, diagnostic reference system that combines the canonical timeline with an explicit relational graph over event versions. Its purpose is diagnostic: it lets us test whether a relational prior over versions (and temporal continuity within each source) helps version selection beyond simple heuristics.
...
Algorithm 1 in the paper prescribes computing LexRank-style centrality over the full TAEG (using TF–IDF cosine weights on BEFORE and SAME_EVENT edges, PageRank-style power iteration) and then selecting, per canonical event, the version with maximum centrality within the SAME_EVENT cluster. During the paper revision the implementation of Algorithm 1 was completed and audited.

Transparency note: the system previously reported in earlier pre-revision artifacts as "TAEG-LexRank" had a mismatch: the code built the graph but selected per-event versions by text length (the longest version). The revision implements correct centrality-based selection as `taeg` and preserves the previous longest-text behavior under the label `Timeline+Longest` for comparison.
...
Corpus-level metrics are diluted on this dataset because 72/169 events have a single version; for those events all timeline-aware strategies emit identical text. Selection-level evaluation (oracles, accuracies, percentiles) is reported in `outputs/selection_eval.json`.

### Conciseness vs. Consolidation Analysis

The LexRank baseline table shows multiple sentence-budget settings to demonstrate that increasing summary length does not solve narrative coherence. Timeline-aware strategies consistently achieve τ = 1.000 while improving ROUGE-L substantially.
...
We release the Gospel Consolidation Language Resource used in this work. It contains the English New International Version (NIV, 2011) texts of the four Gospels aligned using a book:chapter:verse scheme and mapped to a canonical 169-event Holy Week chronology.

The system parses any XML where book, chapter and verse identifiers are clearly tagged as attributes. This decouples the chronological structure from translation or language and makes the resource reusable with other texts and translations.
...
Notes: all methods are evaluated by the same evaluator instance against the same Golden Sample; every randomized component uses fixed, logged seeds; every timeline-aware run is verified by the strict event-order monotonicity check.
...
Timeline-aware methods emit events in canonical timeline order by construction; we verify this property per run. Timeline-aware rows report τ = 1.000 when the strict monotonicity check has passed. The heuristic sentence→event matcher is only used to estimate τ for the timeline-agnostic LexRank baseline.
